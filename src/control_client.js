...
export function initControlClient(api) {
    ...
    // Temporarily override window.prompt/confirm for programmatic calls.
    // Serialized via mutex to prevent concurrent overrides from corrupting
    // each other's restore. Necessary because upstream ViperIDE uses browser
    // dialogs and we can't modify those function signatures.
    let dialogLock = Promise.resolve()
    async function withDialogOverrides(overrides, fn) {
        const prev = dialogLock
        let release
        dialogLock = new Promise(r => { release = r })
        await prev
    ...
    async function dispatch(method, params) {
        switch (method) {
            ...
            case 'connect_device': {
                ...
                withDialogOverrides(overrides, () => api.connectDevice(connType)).catch(() => {})
